 ...
 */
public class CyclonBasicPartialView extends AAgingPartialView {
     ...
     */
    public CyclonBasicPartialView(int c, int l) {
        super();
        CyclonBasicPartialView.l = l;
        CyclonBasicPartialView.c = c;
    }
    ...
    public List<Node> getSample(Node caller, Node neighbor, boolean isInitiator) {
        ...
        if (!isInitiator) { // called from the chosen peer
            sampleSize = Math.min(sampleSize, CyclonBasicPartialView.l);
        } else { // called from the initiating peer
            sampleSize = Math.min(sampleSize - 1, CyclonBasicPartialView.l - 1);
    ...
    public void mergeSample(Node me, Node other, List<Node> newSample,
        ...
        while (i >= 0 && this.partialView.size() < CyclonBasicPartialView.c) {
    ...
    protected Object clone() throws CloneNotSupportedException {
        CyclonBasicPartialView cpv = new CyclonBasicPartialView(CyclonBasicPartialView.c,
                CyclonBasicPartialView.l);
