...
import cyclonbasic.CyclonBasicMessage;
import cyclonbasic.CyclonBasicPartialView;
 ...
 * The Snob protocol
 */
public class CyclonBasic extends ARandomPeerSamplingProtocol implements
        IRandomPeerSampling {
    ...
    // #C local variables
    private CyclonBasicPartialView partialView;
     ...
     */
    public CyclonBasic(String prefix) {
        super(prefix);
        CyclonBasic.c = Configuration.getInt(prefix + "." + PAR_C);
        CyclonBasic.l = Configuration.getInt(prefix + "." + PAR_L);
        this.partialView = new CyclonBasicPartialView(CyclonBasic.c, CyclonBasic.l);
    }
    ...
    private static void randomWalk(Node origin, Node current, int ttl) {
        final CyclonBasic originCyclon = (CyclonBasic) origin.getProtocol(CyclonBasic.pid);
        final CyclonBasic currentCyclon = (CyclonBasic) current.getProtocol(CyclonBasic.pid);
    ...
    public void periodicCall() {
        if (this.isUp() && this.partialView.size() > 0) {
            ...
            List<Node> currentPartialView = this.partialView.getPeers();
            Collections.shuffle(currentPartialView, CommonState.r);
            List<Node> currentPartialViewSubset = currentPartialView.subList(0, Math.min(l, currentPartialView.size()));
            Node q = currentPartialViewSubset.get(CommonState.r.nextInt(currentPartialViewSubset.size()));
            CyclonBasic qCyclon = (CyclonBasic) q
    ...
    public IMessage onPeriodicCall(Node origin, IMessage message) {
        ...
        return new CyclonBasicMessage(samplePrime);
    }
    ...
    public void join(Node joiner, Node contact) {
        ...
        if (contact != null) { // the very first join does not have any contact
            CyclonBasic contactCyclon = (CyclonBasic) contact.getProtocol(CyclonBasic.pid);
    ...
    public void onSubscription(Node origin) {
        ...
        int nbRndWalk = Math.min(CyclonBasic.c - 1, aliveNeighbors.size());

        for (int i = 0; i < nbRndWalk; ++i) {
            randomWalk(origin, aliveNeighbors.get(i), CyclonBasic.RND_WALK);
        }
    }
    ...
    public IRandomPeerSampling clone() {
        try {
            CyclonBasic cyClone = new CyclonBasic();
            cyClone.partialView = (CyclonBasicPartialView) this.partialView.clone();
            return cyClone;
